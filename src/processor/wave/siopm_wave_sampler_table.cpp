...
void SiOPMWaveSamplerTable::set_sample(SiOPMWaveSamplerData *p_sample, int p_key_range_from, int p_key_range_to) {
	...
	int key_to = MIN(SiOPMTable::SAMPLER_DATA_MAX - 1, p_key_range_to);

	if (key_to == -1) {
		key_to = key_from;
	}

	ERR_FAIL_COND_MSG(key_from > (SiOPMTable::SAMPLER_DATA_MAX - 1), vformat("SiOPMWaveSamplerTable: Invalid sample key range, left boundary cannot be greater than %d but %d was given.", (SiOPMTable::SAMPLER_DATA_MAX - 1), key_from));
...
SiOPMWaveSamplerTable::SiOPMWaveSamplerTable() :
	...
	_table.resize_zeroed(SiOPMTable::SAMPLER_DATA_MAX);
	clear();
}
