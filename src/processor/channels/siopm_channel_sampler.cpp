		...
		if (_has_effect_send) {
			for (int i = 0; i < SiOPMModule::STREAM_SEND_SIZE; i++) {
				if (_volumes[i] > 0) {
					...
					if (stream) {
						double volume = _volumes[i] * _expression * _chip->get_sampler_volume();
			...
			double volume = _volumes[0] * _expression * _chip->get_sampler_volume();
			stream->write_from_vector(_sample_data->get_wave_data(), _sample_index, _buffer_index, processed, volume, _sample_pan, _sample_data->get_channel_count());
		}
...
SiOPMChannelSampler::SiOPMChannelSampler(SiOPMModule *p_chip) : SiOPMChannelBase(p_chip) {
	// Empty.
}
