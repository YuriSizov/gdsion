...
SiOPMModule *SiOPMChannelManager::_chip = nullptr;
HashMap<SiOPMChannelManager::ChannelType, SiOPMChannelManager *> SiOPMChannelManager::_channel_managers;

void SiOPMChannelManager::initialize(SiOPMModule *p_chip) {
	_chip = p_chip;
...
SiOPMChannelBase *SiOPMChannelManager::_create_channel(SiOPMChannelBase *p_prev, int p_buffer_index) {
	...
	if (_terminator->_next->_is_free) {
		...
		switch (_channel_type) {
			case CT_CHANNEL_FM: {
				new_channel = memnew(SiOPMChannelFM(_chip));
			} break;
			case CT_CHANNEL_PCM: {
				new_channel = memnew(SiOPMChannelPCM(_chip));
			} break;
			case CT_CHANNEL_SAMPLER: {
				new_channel = memnew(SiOPMChannelSampler(_chip));
			} break;
			case CT_CHANNEL_KS: {
				new_channel = memnew(SiOPMChannelKS(_chip));
			} break;
...
SiOPMChannelManager::SiOPMChannelManager(ChannelType p_channel_type) {
	...
	_terminator = new SiOPMChannelBase(_chip);
