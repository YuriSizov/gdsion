...
void SiOPMOperator::set_pipes(SinglyLinkedList<int> *p_out_pipe, SinglyLinkedList<int> *p_in_pipe, bool p_final) {
	...
	_in_pipe = p_in_pipe ? p_in_pipe : _chip->get_zero_buffer();
	_base_pipe = (p_out_pipe != p_in_pipe) ? p_out_pipe : _chip->get_zero_buffer();
}
...
void SiOPMOperator::initialize() {
	...
	_base_pipe = _chip->get_zero_buffer();
	_feed_pipe->value = 0;

	// Reset all parameters.
	set_operator_params(_chip->get_init_operator_params());
