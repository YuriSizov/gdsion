...
SiMMLSimulatorGB::SiMMLSimulatorGB() :
		SiMMLSimulatorBase(MT_GB, 4, memnew(SiMMLSimulatorVoiceSet(11))) {
	// Default voice set.

	for (int i = 0; i < 8; i++) {
		_default_voice_set->set_voice(i, memnew(SiMMLSimulatorVoice(PG_PULSE + i * 2, PT_PSG)));
	}
	_default_voice_set->set_voice(8, memnew(SiMMLSimulatorVoice(PG_CUSTOM, PT_PSG)));
	_default_voice_set->set_voice(9, memnew(SiMMLSimulatorVoice(PG_NOISE_PULSE, PT_GB_NOISE)));
	_default_voice_set->set_voice(10, memnew(SiMMLSimulatorVoice(PG_NOISE_GB_SHORT, PT_GB_NOISE)));
	...
	tone_voice_set->set_voice(0, memnew(SiMMLSimulatorVoice(PG_NOISE_PULSE, PT_GB_NOISE)));
	tone_voice_set->set_voice(1, memnew(SiMMLSimulatorVoice(PG_NOISE_GB_SHORT, PT_GB_NOISE)));
	tone_voice_set->set_init_voice_index(0);
