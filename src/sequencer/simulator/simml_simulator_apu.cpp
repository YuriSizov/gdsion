...
SiMMLSimulatorAPU::SiMMLSimulatorAPU() :
		SiMMLSimulatorBase(SiMMLTable::MT_APU, 4, memnew(SiMMLSimulatorVoiceSet(11))) {
	// Default voice set.

	for (int i = 0; i < 8; i++) {
		_default_voice_set->set_voice(i, memnew(SiMMLSimulatorVoice(SiOPMTable::PG_PULSE + i * 2, SiOPMTable::PT_PSG)));
	}
	_default_voice_set->set_voice(8, memnew(SiMMLSimulatorVoice(SiOPMTable::PG_TRIANGLE_FC, SiOPMTable::PT_PSG)));
	_default_voice_set->set_voice(9, memnew(SiMMLSimulatorVoice(SiOPMTable::PG_NOISE_PULSE, SiOPMTable::PT_PSG_NOISE)));
	_default_voice_set->set_voice(10, memnew(SiMMLSimulatorVoice(SiOPMTable::PG_NOISE_SHORT, SiOPMTable::PT_PSG_NOISE)));
	...
	tone_voice_set->set_voice(0, memnew(SiMMLSimulatorVoice(SiOPMTable::PG_TRIANGLE_FC, SiOPMTable::PT_PSG)));
	...
	tone_voice_set->set_voice(0, memnew(SiMMLSimulatorVoice(SiOPMTable::PG_NOISE_PULSE, SiOPMTable::PT_PSG_NOISE)));
	tone_voice_set->set_voice(1, memnew(SiMMLSimulatorVoice(SiOPMTable::PG_NOISE_SHORT, SiOPMTable::PT_PSG_NOISE)));
	tone_voice_set->set_init_voice_index(0);
