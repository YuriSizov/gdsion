...
void SiMMLData::register_all() {
	...
	SiOPMTable::get_instance()->sampler_tables[0]->set_stencil(_sampler_tables[0]);
	SiOPMTable::get_instance()->sampler_tables[1]->set_stencil(_sampler_tables[1]);

	SiOPMTable::get_instance()->set_stencil_custom_wave_tables(_wave_tables);
	SiOPMTable::get_instance()->set_stencil_pcm_voices(_pcm_voices);

	SiMMLTable::get_instance()->set_stencil_envelopes(_envelope_tables);
	SiMMLTable::get_instance()->set_stencil_voices(_fm_voices);
}

// Tables.

SiMMLEnvelopeTable *SiMMLData::get_envelope_table(int p_index) const {
	ERR_FAIL_INDEX_V(p_index, SiMMLTable::ENVELOPE_TABLE_MAX, nullptr);

	return _envelope_tables[p_index];
}

void SiMMLData::set_envelope_table(int p_index, SiMMLEnvelopeTable *p_envelope) {
	ERR_FAIL_INDEX(p_index, SiMMLTable::ENVELOPE_TABLE_MAX);

	_envelope_tables.write[p_index] = p_envelope;
}

SiOPMWaveTable *SiMMLData::get_wave_table(int p_index) const {
	int index = p_index & (SiOPMTable::WAVE_TABLE_MAX - 1);
...
SiOPMWaveTable *SiMMLData::set_wave_table(int p_index, Vector<double> p_data) {
	...
	for (int i = 0; i < p_data.size(); i++) {
		log_table.write[i] = SiOPMTable::calculate_log_table_index(p_data[i]);
	}
...
SiOPMWaveSamplerTable *SiMMLData::get_sampler_table(int p_index) const {
	ERR_FAIL_INDEX_V(p_index, SiOPMTable::SAMPLER_TABLE_MAX, nullptr);

	return _sampler_tables[p_index];
}

void SiMMLData::set_sampler_table(int p_index, SiOPMWaveSamplerTable *p_sampler) {
	ERR_FAIL_INDEX(p_index, SiOPMTable::SAMPLER_TABLE_MAX);
...
void SiMMLData::set_voice(int p_index, SiMMLVoice *p_voice) {
	ERR_FAIL_INDEX(p_index, SiMMLTable::VOICE_MAX);
...
SiMMLVoice *SiMMLData::get_pcm_voice(int p_index) {
	int index = p_index & (SiOPMTable::PCM_DATA_MAX - 1);
...
void SiMMLData::clear() {
	...
	for (int i = 0; i < SiMMLTable::ENVELOPE_TABLE_MAX; i++) {
		_envelope_tables.write[i] = nullptr;
	}
	for (int i = 0; i < SiMMLTable::VOICE_MAX; i++) {
		_fm_voices.write[i] = nullptr;
	}

	for (int i = 0; i < SiOPMTable::WAVE_TABLE_MAX; i++) {
		if (_wave_tables[i]) {
			_wave_tables[i]->free();
			_wave_tables.write[i] = nullptr;
		}
	}
	for (int i = 0; i < SiOPMTable::PCM_DATA_MAX; i++) {
		if (_pcm_voices[i]) {
...
SiMMLData::SiMMLData() {
	_envelope_tables.resize_zeroed(SiMMLTable::ENVELOPE_TABLE_MAX);
	_wave_tables.resize_zeroed(SiOPMTable::WAVE_TABLE_MAX);
	_sampler_tables.resize_zeroed(SiOPMTable::SAMPLER_TABLE_MAX);

	_fm_voices.resize_zeroed(SiMMLTable::VOICE_MAX);
	_pcm_voices.resize_zeroed(SiOPMTable::PCM_DATA_MAX);
